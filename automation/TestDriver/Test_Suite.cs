...
namespace echoAutomatedSuite
{
    public class TestSuite
    {
            ...
            public static string getXlPath(string inString)
            {
                ...
                switch (inString)
                {
                    ...
                    case "Strive Regressions":
                        retString = Application.StartupPath + "\\Data Sheets\\Strive Regressions.xlsm";
                        break;
        ...
        #region Private Functions
            ...
            #region getXLData
            public static string[,] getXLData(string tstName, String xlPath, string shtName, ref string product)
            {
                ...
                for (int x = 2; x <= numRows; x++)
                {
                    if (wrkSheet.Cells[x, 1].Value == tstName)
                    {
                        ...
                        //set the product from the master sheet row 
                        product = wrkSheet.Cells[x, 3].Value;
                ...
                tmpArray = new string[numCols - 3];

                //set arrString to a two dimesional array that will contain the 
                arrString = new string[numCols - 3, 3];
            ...
            private static string[] getArray(Excel.Worksheet wrkSheet, int numRows, int numCols)
            {
                ...
                outArray = new string[numCols - 3];

                for (int x = 0; x < numCols - 3; x++)
                {
                    outArray[x] = (string)wrkSheet.Cells[numRows, x + 4].Value;
                }
                return outArray;
            }
            #endregion
            ...
            public static string getTableEntry(IWebDriver driver, string tblXpath, string instring, string schString, string schSuffix, int strLen)
            {
                int fndItem;
                int charCount;
                string tempstring;
                ...
                charCount = schString.Length;

                //get the actual entry passed from the xpath sent in from the calling function  
                thsString = driver.FindElement(By.XPath(tblXpath + schSuffix)).Text;

                //construct tempstring to verify against the expeted and actual values
                for (int x = 0; x < instring.Length - charCount; x++)
                {
                    //set thsChar equal to the character being processed
                    tempstring = instring.Substring(x, charCount);

                    if (tempstring.Trim() != schString)
                        thsChar = instring.Substring(charCount + x, 1);

                    //look for a backslash which indicate a carriage return or newline
                    if ((thsChar == "\r" || thsChar == "\n") || tempstring.Trim() == schString)
                    {
                        break;
                    }
                }
                ...
                if (tempstring == "")
                    fmtInString = instring;
                else
                    fmtInString = tempstring.Trim();
                tempstring = "";

                for (int x = 0; x < thsString.Length - charCount; x++)
                {
                    //set thsChar equal to the character being processed
                    tempstring = thsString.Substring(x, charCount);

                    if (tempstring.Trim() != schString)
                        thsChar = thsString.Substring(charCount + x, 1);
                ...
                if (tempstring == "")
                    fmtThsString = thsString;
                else
                    fmtThsString = tempstring.Trim();
                tempstring = "";

                if ((fmtInString == schString) && (fmtInString == fmtThsString))
                {
                    fndItem = 1;
                }
                else
            ...
            #region splitList
            public static string[] splitList(string instring, ref string count)
            {
                string[] outstring;
                string thsChar;
                string tempstring;
                ...
                //set the oustring array
                outstring = new string[strCount];

                //split the string into the outstring array
                for (int x = 0; x < strlen; x++)
                {
                    thsChar = instring.Substring(x, 1);
