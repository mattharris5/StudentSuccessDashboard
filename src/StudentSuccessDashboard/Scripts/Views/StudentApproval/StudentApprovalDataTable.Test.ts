...
test("When RenderOptOut, Then render checkbox to update opt-out flag", function () {
    var expected: string = "<input class='optOut-checkbox' type='checkbox' data-value='1' />";
    var full: any = {
        Id: 1
    };
    var target: StudentApprovalDataTable = new StudentApprovalDataTable();

    var actual: string = target.RenderOptOut(false, null, full);

    strictEqual(actual, expected);
});

test("Given student has parental opt-out, When RenderOptOut, Then checkbox to update opt-out flag is checked", function () {
    var expected: string = "<input class='optOut-checkbox' type='checkbox' data-value='1' checked='CHECKED' />";
    var full: any = {
        Id: 1
    };
    var target: StudentApprovalDataTable = new StudentApprovalDataTable();

    var actual: string = target.RenderOptOut(true, null, full);
...
test("Given table is in print export mode, When RenderOptOut, Then return raw boolean string", function () {
    ...
    var actual: string = target.RenderOptOut(false, null, null);
