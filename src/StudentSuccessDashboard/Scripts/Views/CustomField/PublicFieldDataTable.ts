...
class PublicFieldDataTable extends DataTable {
    constructor() {
        super($('#publicFields'));
    }
    ...
    private RenderActions(data: any, type: string, full: any): string {
        var buttonOutput = '<ul class="buttons clearfix"><li>' +
                        '<a class ="btn btn-primary btn-mini editPublicFields" role="button" data-value="{0}"><i class ="icon-edit"></i>Edit</a>' +
                        '</li><li>' +
                        '<a class="btn btn-danger btn-mini deletePublicFields" role="button" data-value="{0}"><i class ="icon-remove"></i>Delete</a>' +
                        '</li></ul>';
