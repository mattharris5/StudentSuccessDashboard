...
class StudentDataTable extends DataTable {
    private _StudentSelector: ISelector;

    constructor(studentSelector: ISelector) {
        super($('#students'));
    ...
    GetTableLanguage() {
        return {
            "sLengthMenu": "_MENU_ records per page <span class='legend'><sup><i class='icon-asterisk'></i></sup><strong>Service Request Priority:</strong>" +
                           "&nbsp;<i class='icon-sign-blank red' title='High'></i>=High &nbsp;<i class='icon-sign-blank orange' title='Medium'></i>=Medium &nbsp;" +
                           "<i class='icon-sign-blank green' title='Low'></i>=Low  &nbsp;<i class='icon-sign-blank grey' title='None'></i>=None</span>"
        }
    }

    RenderServiceRequest(oObj): string {
        var values: string[] = oObj.aData[0].split("|");
        var valList: string = "";
        if (values.length > 1) {
            valList = '<ul class="flags">';
            for (var i = 0; i < values.length; i++) {
                if (values[i] == "Y") {
                    var outputButton: string = "";
                    var priority: string = "";
                    if (DataTable.IsInPrintExportMode) {
                        ...
                        outputButton =
                            ...
                            '<a class="btn btn-mini {0} dropdown-toggle" data-toggle="dropdown">{1}{2}{4}<span class="caret"></span></a>' +
                            '<ul class="dropdown-menu pull-right"><li><a href="#" class="editServiceRequest" data-value="{3}"><i class="icon-edit"></i> Edit This Service Request</a></li>' +
                            '<li><a href="#" class="removeServiceRequest" data-value="{3}"><i class="icon-remove"></i> Remove This Service Request</a></li></ul></div></li>';
                ...
                else if (values[i] == "N")
                {
                    ...
                    var priority: string = StudentDataTable.GetPriorityButtonStyleClass(values[i + 3]);
                    valList += StringExtensions.format(outputButton, priority, values[i + 2], values[i + 4].length > 0 ? "/" + values[i + 4] : "", values[i + 1], values[i + 5] == "2" ? '<i class="icon-ok FulfillmentStatus"></i>' : values[i + 5] == "3" ? '<i class="icon-remove FulfillmentStatus"></i>' : "");
                }
                i = i + 5;
            }
    ...
    private static GetPriorityButtonStyleClass(value: string): string {
        if (value == "2")
            return "btn-success";
        else if (value == "3")
            return "btn-warning";
    ...
    private RenderName(oObj): string {
        var values: string[] = oObj.aData[2].split("|");
        if (values.length > 1) {
    ...
    private static BuildServiceOfferingsFormatString(displayCode: string): string {
        if (displayCode === "Y") {
            if (DataTable.IsInPrintExportMode) {
                return "<li>{0}</li>";
            }
            return "<li><div class='btn-group'><a class='btn btn-mini dropdown-toggle' data-toggle='dropdown' href='#'>{0} <span class='caret'></span></a>" +
                    "<ul class='dropdown-menu pull-right'><li><a href='#' class='editAssignedServiceOffering' data-toggle='modal' data-value='{1}'><i class='icon-edit'></i> Edit This Service Offering</a></li>" +
                    "<li><a href='#' class='removeAssignedServiceOffering' data-toggle='modal' data-value='{1}'><i class='icon-remove'></i> Remove This Service Offering</a></li>" +
                    "<li><a href='/ServiceAttendance/Index/{1}'><i class='icon-eye-open'></i> View Attendance for this Service Offering</a></li></ul></div></li>";
        }
        if (displayCode == "N") {
    ...
    private RenderServiceOfferings(oObj): string {
        var values: string[] = oObj.aData[5].split("|");
        var valList = "";
        if (values.length > 1) {
    ...
    private RenderActions(oObj): string {
