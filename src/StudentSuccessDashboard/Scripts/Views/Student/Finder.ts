...
class StudentFinderPage extends ManagePage {
    private _StudentSelector: ISelector;
    private _StudentDataTable: IDataTable;
    private _StudentFilter: IFilter;
    ...
    SetupControls(): void {
        var instance: StudentFinderPage = this;
        ...
        $('#serviceRequestFlagModal').on('click', function () {
            ...
            CrudUtilities.DisplayCrudModal(instance._StudentDataTable.DataTableElement, '/ServiceRequest/Create', '/ServiceRequest/Create',
                'Request Service', 'Apply Service Request(s)', '#flagModalBody',
                function () {
    ...
    SetupModals(): void {
        ...
        CrudUtilities.WireupCrudModal(this._StudentDataTable.DataTableElement, CrudUtilities.MainWrapSelector,
            '#tbodyStudents tr .removeServiceRequest', '/ServiceRequest/Delete/', true, 'Delete Service Request', 'Remove', '#flagModalBody', function () {
        ...
        CrudUtilities.WireupCrudModal(this._StudentDataTable.DataTableElement, CrudUtilities.MainWrapSelector,
            '#tbodyStudents tr .editServiceRequest', '/ServiceRequest/Edit/', true, 'Edit Service Request', 'Edit Service Request', '#flagModalBody', function () {
                CrudUtilities.ModalTitleElement.text('Edit ' + $('#SelectedServiceTypeId :selected').text() + ' / ' + $('#SelectedSubjectId :selected').text());
            }, null, null);

        CrudUtilities.WireupCrudModal(this._StudentDataTable.DataTableElement, CrudUtilities.MainWrapSelector,
            '#tbodyStudents tr .removeAssignedServiceOffering', '/Service/DeleteScheduledOffering/', true, 'Remove Service Offering', 'Remove Service Offering', '#form-remove', function () {
                CrudUtilities.ModalTitleElement.text('Remove Service Offering ' + $('#scheduledServiceOfferingToDelete').text());
                CrudUtilities.SubmitButton.removeClass('btn-primary').addClass('btn-danger');
            }, null, null);

        CrudUtilities.WireupCrudModal(this._StudentDataTable.DataTableElement, CrudUtilities.MainWrapSelector,
            '#tbodyStudents tr .editAssignedServiceOffering', '/Service/EditScheduledOffering/', true, 'Edit Service Offering', 'Edit Service Offering', '#form-addedit', function () {
