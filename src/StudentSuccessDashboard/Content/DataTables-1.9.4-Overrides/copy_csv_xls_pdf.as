...
/* See: http://www.buzzbrainwave.com/2012/11/datatables-tabletools-styling.html */
package {
	...
	public class ZeroClipboard extends Sprite {
		...
		private function clickHandler(event:Event):void {
			...
			if ( action == "pdf" ) {
				...
				var formattedClipText:String = clipText.replace(/<br \/>/g, " ");
				if ( action == "save" ) {
					/* Save as a file */
					if ( charSet == 'UTF16LE' ) {
						fileRef.save( strToUTF16LE(formattedClipText), fileName );
					} else {
						fileRef.save( strToUTF8(formattedClipText), fileName );
					}
				} else {
					/* Copy the text to the clipboard. Note charset and BOM have no effect here */
					System.setClipboard( formattedClipText );
					ExternalInterface.call( 'ZeroClipboard_TableTools.dispatch', domId, 'complete', formattedClipText );
				}
			}
		}
		...
		private function configPdf():PDF
		{
			...
			for ( i=0, iLen=dataIn.length ; i<iLen ; i++ )
			{
				if ( dataIn[i] != "" )
				{
					dataOut.push( dataIn[i].replace(/<br \/>/g, "\n").split("\t") );
				}
			}
